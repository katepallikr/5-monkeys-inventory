...
For a sales import to actually deplete inventory (not just create an empty placeholder `Recipe`), the menu item needs a `Recipe` whose name exactly matches the sales report's item name, with `RecipeIngredient` rows pointing at real catalog items. `prisma/seed-drink-recipes.ts`, `prisma/seed-remaining-items.ts`, and `prisma/seed-house-cocktails.ts` set this up for the bar/drink menu (re-run any of them with `npx tsx prisma/<file>.ts` - all idempotent). Depletion patterns used, depending on how the catalog item is tracked:

- **Whole units** (bottled beer, canned RTDs, sodas) - 1 sale depletes 1 unit of the item's own `unitType`.
- **Fractional units** (spirits poured as a shot, draft beer poured from a keg, cocktail ingredients poured from a bottle) - depletes `pourSize / containerSize` per sale, computed from the item's `pourSizeOz`/`bottleVolumeMl` (or a keg-size constant for draft). This matters because `onHand` for these items is tracked in whole bottles/kegs, not ounces - depleting by a flat oz number instead of the fraction would drain a bottle roughly 15-30x too fast. Hookah flavor tobacco sidesteps the container-size question entirely by tracking `onHand` directly in `GRAM`s instead of canisters.
- **Mixed cocktails** (`seed-house-cocktails.ts`) - the House Cocktails menu only lists ingredients, not pour sizes, so quantities use a confirmed default convention: 1.5oz for the first spirit listed (base), 0.5oz for each additional spirit/liqueur, 0.75oz per juice/syrup/mixer, ~1oz for a described "splash", and a full container for "topped with a can of X". Garnishes and dash-based bitters are intentionally not tracked. Reuses existing catalog items where the same bottle is shared with kitchen prep (e.g. lime juice, agave syrup) rather than creating bar-only duplicates.

Still not covered by these scripts (need more input from the business owner, not a guess): "Flights" (a 3-margarita sampler - which 3 and in what ratio isn't specified), `Green Tea Jameson` and `MexicanCandies` (not on the House Cocktails page, need their own description/screenshot), and the pre-existing `Jack Daniels Shot` recipe/duplicate catalog entries.
